    ...
    const fetchMemes = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Backend returns a single meme object or { message: '...' }
          if (data && !data.message && data.id) {
            setMemes([{
              id: data.id,
              imageSrc: data.imageUrl,
              postedBy: data.user?.username || "Anonymous",
              likes: 0, // Backend doesn't persist likes for memes yet
              liked: false,
              reported: false,
            }]);
          } else {
      ...
      <main className={styles.main}>
        <div className={styles.boardContainer}>
          ...
          <div className={styles.memesWrapper}>
            {memes.length === 0 ? (
                <div style={{ position: 'relative', zIndex: 10, padding: '2rem', textAlign: 'center', color: '#5a4636' }}>
